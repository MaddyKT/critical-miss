    ...
    [
      {
        ...
        dc: 14,
        onSuccess: (ch) => ({
          c: advanceArc(setArcFlag({ ...ch, xp: ch.xp + 6, party: { ...ch.party, inParty: true, members: [...new Set([...ch.party.members, 'Chesty'])] } }, 'mimic_party'), 20),
        ...
        onFail: (ch) => ({
          c: advanceArc(setArcFlag({ ...ch, hp: clamp(ch.hp - 2, 0, ch.maxHp), xp: ch.xp + 4 }, 'mimic_party'), 16),
